...
import (
	...
	"github.com/jaredwsimmons/google-messages-mcp/cmd"
)
...
func main() {
	...
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: gmessages <pair|serve|demo|read|thread|threads|send|import>")
	...
	case "read", "search":
		if len(os.Args) < 3 {
			fmt.Fprintln(os.Stderr, "Usage: gmessages read <query> [--limit N] [--phone NUMBER] [--json]")
			os.Exit(1)
		}
		err = cmd.RunRead(logger, os.Args[2:]...)
	case "thread":
		if len(os.Args) < 3 {
			fmt.Fprintln(os.Stderr, "Usage: gmessages thread <name|number|conversation_id> [--limit N] [--since YYYY-MM-DD] [--until YYYY-MM-DD] [--json]")
			os.Exit(1)
		}
	...
	case "send":
		if len(os.Args) < 4 {
			fmt.Fprintln(os.Stderr, "Usage: gmessages send <conversation_id> <message>")
			os.Exit(1)
		}
		err = cmd.RunSend(logger, os.Args[2], os.Args[3])
	case "send-group":
		if len(os.Args) < 4 {
			fmt.Fprintln(os.Stderr, "Usage: gmessages send-group <phone1,phone2,...> <message>")
			os.Exit(1)
		}
	...
	case "import":
		if len(os.Args) < 3 {
			fmt.Fprintln(os.Stderr, "Usage: gmessages import <gchat|gchat-conversation|whatsapp|signal> [args...]")
			os.Exit(1)
		}
		err = cmd.RunImport(logger, os.Args[2], os.Args[3:])
	case "debug-media":
		if len(os.Args) < 3 {
			fmt.Fprintln(os.Stderr, "Usage: gmessages debug-media <conversation_id>")
			os.Exit(1)
		}
